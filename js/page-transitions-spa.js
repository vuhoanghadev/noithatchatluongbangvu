      ...
      fetch(fullUrl)
        ...
        .then((html) => {
          ...
          pageCache[fullUrl] = {
            ...
            scripts: Array.from(doc.querySelectorAll('script:not([src])')).map(
...
function startPageTransition(url, x, y, isHistoryNavigation = false) {
  ...
  const loadingTimeout = setTimeout(
    ...
    isCached ? 1000 : 500
  ); // Longer delay for cached pages as they should load faster
    ...
    fetch(fullUrl)
      ...
      .then((html) => {
        ...
        pageCache[fullUrl] = {
          ...
          scripts: Array.from(
            newDocument.querySelectorAll('script:not([src])')
        ...
        completeTransition(
          ...
          Array.from(newDocument.querySelectorAll('script:not([src])')).map(
            (script) => script.textContent
          )
        );
      })
