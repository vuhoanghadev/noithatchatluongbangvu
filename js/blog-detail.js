...
function updateMetaTags(post) {
    ...
    document.querySelector('meta[property="og:title"]').setAttribute('content', `${post.title} - Nội Thất Chất Lượng Bàng Vũ`);
    ...
    document.querySelector('meta[property="twitter:title"]').setAttribute('content', `${post.title} - Nội Thất Chất Lượng Bàng Vũ`);
    document.querySelector('meta[property="twitter:description"]').setAttribute('content', post.excerpt);
...
function updatePostNavigation(currentPostId) {
    ...
    const currentIndex = uniquePosts.findIndex(post => post.id === currentPostId);
...
function displayRelatedProducts(relatedProductIds) {
    ...
    const products = [
        {
            ...
            image: 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8ZnVybml0dXJlfGVufDB8fDB8fHww',
            promotion: 'Giảm 20%'
        },
        {
            id: 2,
            name: 'Bàn Ăn Gỗ Tự Nhiên',
            image: 'https://images.unsplash.com/photo-1538688525198-9b88f6f53126?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTB8fGZ1cm5pdHVyZXxlbnwwfHwwfHx8MA%3D%3D',
            promotion: ''
        },
        {
            id: 3,
            name: 'Tủ Quần Áo 4 Cánh',
            image: 'https://images.unsplash.com/photo-1594026112284-02bb6f3352fe?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTR8fGZ1cm5pdHVyZXxlbnwwfHwwfHx8MA%3D%3D',
            promotion: 'Mới'
        },
        {
            id: 4,
            name: 'Kệ Tivi Phòng Khách',
            image: 'https://images.unsplash.com/photo-1493663284031-b7e3aefcae8e?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZnVybml0dXJlfGVufDB8fDB8fHww',
            promotion: ''
        },
        {
            id: 5,
            name: 'Giường Ngủ Gỗ Sồi',
            image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fGZ1cm5pdHVyZXxlbnwwfHwwfHx8MA%3D%3D',
            promotion: 'Giảm 15%'
        },
        {
            id: 6,
            name: 'Bàn Làm Việc Thông Minh',
            image: 'https://images.unsplash.com/photo-1449247709967-d4461a6a6103?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8N3x8ZnVybml0dXJlfGVufDB8fDB8fHww',
            promotion: ''
        },
        {
            id: 7,
    ...
    container.innerHTML = productsToShow.map(product => `
        <div class="card">
            ...
            <h4>${product.name}</h4>
            ${product.promotion ? `<span class="promo-badge">${product.promotion}</span>` : ''}
            <a href="product-details.html?id=${product.id}" class="btn-details">Xem chi tiết</a>
        </div>
    `).join('');
}
...
function displayRelatedPosts(currentPostId, category) {
    ...
    const relatedPosts = uniquePosts.filter(post => 
        post.id !== currentPostId && post.category === category
    );
...
function initSocialShare(post) {
    ...
    window.shareOnPinterest = function() {
        const url = `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(currentUrl)}&media=${encodeURIComponent(post.image)}&description=${encodeURIComponent(post.title)}`;
        window.open(url, '_blank', 'width=600,height=400');
    };
}
